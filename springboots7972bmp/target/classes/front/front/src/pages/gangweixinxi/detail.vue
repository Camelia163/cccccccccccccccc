	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/gangweixinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">岗位编号</div>
						<div class="text "  >{{detail.gangweibianhao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">时薪/小时</div>
						<div class="text "  >{{detail.shixin}}</div>
					</div>
					<div class="item">
						<div class="lable">截止日期</div>
						<div class="text "  >{{detail.jiezhiriqi}}</div>
					</div>
					<div class="item">
						<div class="lable">单位账号</div>
						<div class="text "  >{{detail.danweizhanghao}}</div>
					</div>
					<div class="item">
						<div class="lable">单位名称</div>
						<div class="text "  >{{detail.danweimingcheng}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('gangweixinxi','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('gangweixinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('gangweixinxi','立即申请')" 
							@click="onAcross('gangweishenqing','','','','')" type="warning">
							立即申请
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('gangweixinxi','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="岗位要求" name="first">
					<div class="ql-snow ql-editor" v-html="detail.gangweiyaoqiu"></div>
				</el-tab-pane>
				<el-tab-pane label="岗位简介" name="first1">
					<div class="ql-snow ql-editor" v-html="detail.gangweijianjie"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`gangweixinxi`);
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('gangweixinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'gangweixinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/gangweixinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/gangweixinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此岗位信息？') .then(_ => {
					this.$http.post('gangweixinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {

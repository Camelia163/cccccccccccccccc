<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="select" v-if="type!='info'" label="岗位名称" prop="gangweimingcheng" >
					<el-select :disabled="ro.gangweimingcheng" @change="gangweimingchengChange" v-model="ruleForm.gangweimingcheng" placeholder="请选择岗位名称">
						<el-option
				...
				<el-form-item class="input" v-else-if="ruleForm.gangweimingcheng" label="岗位名称" prop="gangweimingcheng" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="时长/小时" prop="shizhang" >
					<el-input-number v-model="ruleForm.shizhang" placeholder="时长/小时" :disabled="ro.shizhang"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="时长/小时" prop="shizhang" >
					<el-input v-model="ruleForm.shizhang" placeholder="时长/小时" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="总工资" prop="zonggongzi" >
					<el-input v-model="zonggongzi" placeholder="总工资" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.zonggongzi" label="总工资" prop="zonggongzi" >
					<el-input v-model="ruleForm.zonggongzi" placeholder="总工资" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.lurushijian" label="录入时间" prop="lurushijian" >
					<el-input v-model="ruleForm.lurushijian" placeholder="录入时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="单位账号" prop="danweizhanghao" style="display:none">
					<el-input v-model="ruleForm.danweizhanghao" placeholder="单位账号" clearable  :readonly="ro.danweizhanghao"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="单位账号" prop="danweizhanghao" style="display:none">
	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					...
					shizhang: [
						{ required: true, message: '时长/小时不能为空', trigger: 'blur' },
						{ validator: validateNumber, trigger: 'blur' },
					],
					zonggongzi: [
						{ validator: validateNumber, trigger: 'blur' },
		...
		methods: {
			...
			gangweimingchengChange () {
				this.$http({
					url: `follow/gangweixinxi/gangweimingcheng?columnValue=`+ this.ruleForm.gangweimingcheng,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.shixin){
							this.ruleForm.shixin = data.data.shixin
						}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `gongzifafang/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.gongzifafangCrossAddOrUpdateFlag = false;
											this.parent.search();

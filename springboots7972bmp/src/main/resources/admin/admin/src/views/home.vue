<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('gangweixinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{gangweixinxiCount}}</div>
					<div class="name">岗位信息总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 3 -->
		<div class="type3">
			<div id="gangweixinxiChart1" class="echarts1 animate__animated" v-if="isAuth('gangweixinxi','首页统计')"></div>
			<div id="gangweixinxiChart2" class="echarts2 animate__animated" v-if="isAuth('gangweixinxi','首页统计')"></div>
			<div id="gangweixinxiChart3" class="echarts3 animate__animated" v-if="isAuth('gangweixinxi','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getgangweixinxiCount();
		if(this.isAuth('gangweixinxi','首页统计')){
			this.gangweixinxiChat1();
		}
		if(this.isAuth('gangweixinxi','首页统计')){
			this.gangweixinxiChat2();
		}
		if(this.isAuth('gangweixinxi','首页统计')){
			this.gangweixinxiChat3();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'gangweixinxiChart1',css:'animate__'},
				{id:'gangweixinxiChart2',css:'animate__'},
				{id:'gangweixinxiChart3',css:'animate__'},
			]
		...
		getgangweixinxiCount() {
			this.$http({
				url: `gangweixinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.gangweixinxiCount = data.data
				}
			})
		},
// 1234 竖
		gangweixinxiChat1() {
			this.$nextTick(()=>{
				...
				var gangweixinxiChart1 = echarts.init(document.getElementById("gangweixinxiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						gangweixinxiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							gangweixinxiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		gangweixinxiChat2() {
			this.$nextTick(()=>{
				...
				var gangweixinxiChart2 = echarts.init(document.getElementById("gangweixinxiChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						gangweixinxiChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							gangweixinxiChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		gangweixinxiChat3() {
			this.$nextTick(()=>{
				...
				var gangweixinxiChart3 = echarts.init(document.getElementById("gangweixinxiChart3"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						gangweixinxiChart3.setOption(option);
						//根据窗口的大小变动图表
						window.onresize = function() {
							gangweixinxiChart3.resize();
						};
					}else{
						this.$message({
							message: data.msg,

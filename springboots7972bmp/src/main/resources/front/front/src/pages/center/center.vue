	...
	<div class="center-preview">
		...
		<div class="center-info">
			...
			<div class="info-item4" v-if="userTableName=='xuesheng'">
				...
				<div class="text">{{sessionForm.dianhuahaoma}}</div>
			</div>
			<div class="info-item5" v-if="userTableName=='xuesheng'">
				<span class="icon iconfont "></span>
				<div class="label">专业：</div>
				<div class="text">{{sessionForm.zhuanye}}</div>
			</div>
		...
		<el-tabs class="center-tabs" tab-position="left" @tab-click="handleClick">
			<el-tab-pane label="个人中心">
				<el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="180px">
					...
					<el-form-item class="center-item" v-if="userTableName=='xuesheng'" label="电话号码" prop="dianhuahaoma">
						<el-input v-model="sessionForm.dianhuahaoma" placeholder="电话号码" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='xuesheng'" label="专业" prop="zhuanye">
...
<script>
	...
	export default {
		...
		created() {
			...
			if ('xuesheng' == this.userTableName&&this.rules['xueshengxingming']){
				this.rules['xueshengxingming'].push({ required: true, message: '请输入学生姓名', trigger: 'blur' })
			}else if('xuesheng' == this.userTableName&&!this.rules['xueshengxingming']) {
			...
			init() {
				...
				if ('xuesheng' == this.userTableName) {
					this.$http.get('option/zhuanye/zhuanye', {emulateJSON: true}).then(res => {
						if (res.data.code == 0) {
						  this.dynamicProp.zhuanye = res.data.data;
						  this.$forceUpdate()
						}
					});
				}
